...
df["lateral_retention"] = (
    df["Post Bounce Velocity_z"] /
    df["Pre Bounce Velocity_z"]
)
...
# -----------------------
# 3) Remove 3-sigma outliers (global)
# -----------------------
...
lat_mean = df["lateral_retention"].mean()
lat_std = df["lateral_retention"].std(ddof=0)
...
df = df[
    ...
    (df["bounce_retention"] <= bounce_mean + 3 * bounce_std) &
    (df["lateral_retention"] >= lat_mean - 3 * lat_std) &
    (df["lateral_retention"] <= lat_mean + 3 * lat_std)
].copy()
...
df["lateral_bin_median"] = (
    df
    .groupby(["v_pre_bin", "angle_bin", "Phase"])["lateral_retention"]
    .transform("median")
)
...
pitch_pivot = pitch_pivot[
    (pitch_pivot["deliveries"] > 0) &
    (pitch_pivot["ovrexpr"].notna())
].copy()
...
pitch_pivot = pitch_pivot.dropna(
    subset=[
        "avg_runs_per_ball",
        "ovrexpr",
        "pitch_pace",
...
X_full = pitch_pivot[["ovrexpr", "pitch_pace", "pitch_bounciness", "pitch_grip"]].values
...
for fold, (train_idx, test_idx) in enumerate(kf.split(pitch_pivot), start=1):
    ...
    m_base.fit(X_base[train_idx], y_train)
