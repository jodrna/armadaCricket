import pandas as pd
import numpy as np
from batFunctions import buildRunRatings, buildWktRatings, build_rating_debug_tables
...
# Basic preprocessing
# -------------------------
bat_data['competition'] = np.where(bat_data['competition'].str.contains('ODI', na=False), np.where(bat_data['ballsremaining'] < 84, 'ODI2', 'ODI1'), bat_data['competition'])
...
for x in np.arange(0, 2, 1):
    ...
    if x == 0:
        ...
        recencies_w = lookbacks_player_w[((lookbacks_player_w['competition'] == 'T20I') | (lookbacks_player_w['competition'] == 'tier_2')) & (lookbacks_player_w['host'] == 'West Indies') & (lookbacks_player_w['date'] == lookbacks_player_w['date'].max())].loc[:, ['playerid', 'matchid_2', 'recency_weight', 'balls_faced_2']]
