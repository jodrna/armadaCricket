...
for x in np.arange(0, 2, 1):
    ...
    bowler_names = pd.read_csv(PROJECT_ROOT / 'men/playerRatings/bowlT20Mens/data/bowlDataCombined.csv', parse_dates=['date']).loc[:, ['bowlerid', 'bowler']].drop_duplicates()  # this is done to make sure all names have ratingsT20, even if an id is matched to two names like Shaheen
