...
masterLookup['state_id'] = (
    masterLookup['totalInningWickets']
    + ((121 - masterLookup['inningBallNumber']) / 1000)
).round(3)
masterLookup['state_id'] = masterLookup['state_id'].map(lambda x: f"{x:.3f}")


masterLookup.to_csv('/Users/jordan/Documents/ArmadaCricket/Development/men/expBall&runsToCome/6_masterLookup.csv', index=False)
