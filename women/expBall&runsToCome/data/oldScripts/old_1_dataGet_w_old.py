...
sql_query ='''select  matchid, id, tier, date, year, competition, venue, host, home, away, battingteam, innings, ball, delivery2, runs, score, t_runs, wicket, wickets, t_wickets,
                                 ballsremaining, target, reduced, max_balls, noball, wide, ord, byes, legbyes, innperiod, bowlerwicket, realexprbat, realexpwbat, rating_sample_size, batsmanballs, batsman, nonstriker
...
sql_data.to_csv(PROJECT_ROOT / 'women/expBall&runsToCome/data/sqldataforclean2023_w.csv', index=False)
raw_data = pd.read_csv(PROJECT_ROOT / 'women/expBall&runsToCome/data/sqldataforclean2023_w.csv')
...
raw_data = raw_data[(raw_data['competition'] != 'Charlotte Edwards Cup') | (raw_data['date'] > '2022-01-01')]
raw_data = raw_data[raw_data['competition'] != 'The Hundred Women\'s Competition']


# this is the start of removing reduced games
...
raw_data['remove'] = np.where(raw_data['competition'] == 'The Hundred Women\'s Competition', 0, raw_data['remove'])
...
raw_data['ballsremaining'] = np.where(raw_data['competition'] == 'The Hundred Women\'s Competition', round((100-((np.floor(raw_data['ball'])*5)+((raw_data['ball']-np.floor(raw_data['ball']))*100)-1)), 0),
                                      round((120-((np.floor(raw_data['ball'])*6)+((raw_data['ball']-np.floor(raw_data['ball']))*100)-1)), 0))
# use the hundred only after the powerplay
raw_data = raw_data[(raw_data['ballsremaining'] <= 75) | (raw_data['competition'] != 'The Hundred Women\'s Competition')]
...
# export
raw_data.to_csv(PROJECT_ROOT / 'women/expBall&runsToCome/data/cleanedT20BBB_w.csv', index=False)
