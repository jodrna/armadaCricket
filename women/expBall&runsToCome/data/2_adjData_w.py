...
# bblvalues = pd.read_sql_query("""select over, runs_ratio as bbl_runs_ratio, wkts_ratio as bbl_wkts_ratio from player_ratings.bbl_csv_w""", con=connection)
# #
...
# raw_data = raw_data.loc[:, ['matchid', 'venue', 'innings', 'date', 'host', 'competition', 'id', 'nonstriker', 'battingteam', 'over', 'ballsremaining', 'wickets', 'ord', 'batsman', 'bowler', 'batterid', 'bowlerid', 'innperiod', 'realexprbat', 'realexpwbat', 'realexprbowl', 'realexpwbowl', 'ovrexpr', 'ovrexpw', 'power_surge']]
...
# rar_bbl = raw_data.iloc[::-1].groupby(['matchid', 'innings'], sort=False)['rar_bbl'].transform(lambda x: x.rolling(150, min_periods=1, closed='right').sum()).iloc[::-1].reset_index().fillna(0)
# raw_bbl = raw_data.iloc[::-1].groupby(['matchid', 'innings'], sort=False)['raw_bbl'].transform(lambda x: x.rolling(150, min_periods=1, closed='right').sum()).iloc[::-1].reset_index().fillna(0)
...
lineups = pd.read_sql_query("""select matchid, player as batter, playerid as batterid, team as battingteam, carded from player_ratings.t20_lineups_updated_w where matchid = 1000162218""", con=connection)
...
# his = his.loc[:, ['competition', 'ballsremaining', 'wickets', 'matchid', 'batsman', 'nonstriker', 'battingteam', 'id', 'er_tc_smooth_basic',  'ew_tc_smooth_basic', 'rar_ground_sum', 'raw_ground_sum', 'rar_bowl_sum', 'raw_bowl_sum', 'rar_bbl_sum', 'raw_bbl_sum', 'year']]
