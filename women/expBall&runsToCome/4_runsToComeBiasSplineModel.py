...
simData = pd.read_csv('/Users/jordan/Documents/ArmadaCricket/Development/women/expBall&runsToCome/ballSimsClassRateAdjusted.csv')
...
# declare an external runs to come to be appended too from the internal loop, we're going to adjust each wicket down so that there is no discrepancy between sim and reality
...
for x in range(10):
    ...
    i_situationRunsToCome = i_situationRunsToCome[(situationRunsToCome['sample'] > 40) & (i_situationRunsToCome['totalInningWickets'] == x)]
    ...
    i_situationRunsToCome = i_situationRunsToCome[(i_situationRunsToCome['totalInningWickets'] == x) &
                                                  ((situationRunsToCome['sample'] > 40) | (i_situationRunsToCome['totalInningWickets'] < 2))]
...
# a final change, last balls are odd so just set them to actual
situationRunsToCome_adj2['totalInningRunsToComeSimBiasSpline'] = np.where(situationRunsToCome_adj2['inningBallNumber'] == 120, situationRunsToCome_adj2['totalInningRunsToCome'], situationRunsToCome_adj2['totalInningRunsToComeSimBiasSpline'])
...
# export
masterLookup.to_csv('/Users/jordan/Documents/ArmadaCricket/Development/women/expBall&runsToCome/4_masterLookup.csv', index=False)
