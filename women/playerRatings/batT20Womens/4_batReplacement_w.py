...
def build_features(df, transformers, is_training=True, target_type='run'):
    """
    ...
    is_global_comp = df['competition'].isin(['WT20I', 'tier_2'])

    comp_str = np.where(is_global_comp,
                        df['competition'],
                        df['competition'] + " " + df['H/A_competition']).reshape(-1, 1)
    ...
    if is_training:
        nat_cats = ['England', 'India', 'Australia', 'New Zealand', 'South Africa']
    ...
    # 3. Polynomial Features (Age, Order)
    ...
    # 5. Oppo (basic reverted rating)
    rating_col = 'wkt_rating_2' if target_type == 'run' else 'run_rating_2'

    adjusted_rating = df[rating_col].copy()
    mask = df[ord_col] > 7
    adjusted_rating[mask] = (((1 - adjusted_rating[mask]) / 2) * np.minimum(2, abs(df.loc[mask, ord_col] - 7))) + adjusted_rating[mask]

    df_oppo = pd.DataFrame(adjusted_rating)
    df_oppo.columns = ['oppo']

    # Combine
    X = pd.concat([df_comp, df_nat, df_age, df_exp, df_order, df_oppo], axis=1)
...
for x in np.arange(0, 2, 1):
    ...
    n2h_factors = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/auxiliaries/batN2HFactors.csv')
    ...
    if x == 0:
        ratings = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/outputs/batRatingsJungle.csv', parse_dates=['date'], dtype={'battingteam': str} )

    else:
        ratings = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/outputs/batRatingsRasoi.csv', parse_dates=['date'], dtype={'battingteam': str} )
    ...
    bat_data['balls_faced_career'] += 5
    ratings['balls_faced_career'] += 5

    bat_data = bat_data.merge(ratings[ratings['i_balls_faced'] > 0][['playerid', 'date', 'run_rating', 'wkt_rating']], how='left', on=['playerid', 'date'])
    ...
    # --- Runs Model ---
    X1, transformers = build_features(bat_data, transformers, is_training=True, target_type='run')
    y_run = pd.DataFrame(bat_data['run_ratio'])
    rep_run_ratio_model = sm.OLS(y_run, X1, missing='drop').fit()

    # --- Wickets Model ---
    X2, transformers = build_features(bat_data, transformers, is_training=False, target_type='wkt')
    ...
    bat_data['rep_run_ratio'] = np.where((bat_data['competition'].isin(['WT20I'])) & (bat_data['H/A_competition'] == 'Away'),
                                         bat_data['rep_run_ratio'] * np.minimum(1, bat_data['run_factor'] / allaway_runs),
    ...
    bat_data['rep_wkt_ratio'] = np.where((bat_data['competition'].isin(['WT20I'])) & (bat_data['H/A_competition'] == 'Away'),
                                         bat_data['rep_wkt_ratio'] * np.maximum(1, bat_data['wkt_factor'] / allaway_wkts),
    ...
    ratings['rep_run_ratio'] = np.where((ratings['competition'].isin(['WT20I'])) & (ratings['H/A_competition'] == 'Away'),
                                        ratings['rep_run_ratio'] * np.minimum(1, ratings['run_factor'] / allaway_runs),
                                        ratings['rep_run_ratio'])
    ratings['i_rep_runs'] = ratings['rep_run_ratio'] * ratings['i_realexprbat']

    X4, _ = build_features(ratings, transformers, is_training=False, target_type='wkt')
    ratings.insert(ratings.columns.get_loc("wkt_rating_2") + 1, 'rep_wkt_ratio', rep_wkt_ratio_model.predict(X4))
    ratings['rep_wkt_ratio'] = np.where((ratings['competition'].isin(['WT20I'])) & (ratings['H/A_competition'] == 'Away'),
                                        ratings['rep_wkt_ratio'] * np.maximum(1, ratings['wkt_factor'] / allaway_wkts),
    ...
    # 9. Export
    if x == 0:
        ratings.to_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/outputs/batRatingsJungle2.csv', index=False)
    else:
        ratings.to_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/outputs/batRatingsRasoi2.csv', index=False)
