...
# read data
bat_data = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/data/combinedBatData.csv', parse_dates=['date'])
player_info = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/auxiliaries/playerInfo.csv', parse_dates=['dob'])
ratings = pd.read_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/auxiliaries/batRatingsFor.csv')
...
regions = {'South Africa': 'Africa', 'Zimbabwe': 'Africa', 'Afghanistan': 'Asia', 'Bangladesh': 'Asia', 'India': 'Asia', 'Nepal': 'Asia',
           ...
           'Scotland': 'UK', 'Bermuda': 'WI', 'West Indies': 'WI', 'Hong Kong': 'Asia', 'Malaysia': 'Asia'}
...
# Create a unique game flag: only True for the first (playerid, gameid) where format == 't20', group by player and do cumulative sum
...
bat_data.to_csv(PROJECT_ROOT / 'women/playerRatings/batT20Womens/data/combinedBatDataClean.csv', index=False)
